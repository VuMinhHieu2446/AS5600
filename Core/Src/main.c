...
void send_CDC_number(uint8_t number) {
    // Chuyển số thành một chuỗi ký tự
    char buffer[2]; // Đảm bảo đủ lớn để chứa chuỗi
...
void send_CDC(uint16_t angle_AS1, uint16_t angle_AS2, char *text1, char *text2) {
    // Chuyển số thành chuỗi
    char angle_AS1_str[2];  // Đảm bảo đủ lớn để chứa chuỗi số
    sprintf(angle_AS1_str, "%d", angle_AS1);

    char angle_AS2_str[2];  // Đảm bảo đủ lớn để chứa chuỗi số
    sprintf(angle_AS2_str, "%d", angle_AS2);
    // Tính tổng độ dài của chuỗi số và chuỗi văn bản
    int total_length = strlen(angle_AS1_str) + strlen(angle_AS2_str) + strlen(text1) + strlen(text2) +2;
